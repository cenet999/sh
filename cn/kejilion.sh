...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  116|shadowsocks-rust|ssserver-rust|shadowsocks)
			...
			local app_text="轻量高性能的 Shadowsocks Rust 服务端，支持 TCP 和 UDP。"
			...
			docker_app_install() {
				mkdir -p /home/docker/shadowsocks-rust
				cd /home/docker/shadowsocks-rust

				curl -o /home/docker/shadowsocks-rust/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/shadowsocks-rust-docker-compose.yml
				sed -i "s/SSSERVER_PORT_PLACEHOLDER/${docker_port}/g" /home/docker/shadowsocks-rust/docker-compose.yml

				read -e -p "请输入 Shadowsocks 密码（留空自动生成随机强密码）: " ss_password
				if [ -z "$ss_password" ]; then
					ss_password=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 16)
					echo "已自动生成密码: $ss_password"
				fi
				local ss_password_safe=$(printf '%s' "$ss_password" | sed 's/\\/\\\\/g; s/"/\\"/g')

				cat > /home/docker/shadowsocks-rust/config.json << EOF
{
  "server": "0.0.0.0",
...
EOF
				...
				cat > /home/docker/shadowsocks-rust/client-info.txt << EOF
服务器地址: 请填写你的服务器IP或域名
服务器端口: ${docker_port}
密码: ${ss_password}
...
EOF
			...
			docker_app_update() {
				...
				clear
				echo "更新完成"
				echo "当前客户端参数保存在: /home/docker/shadowsocks-rust/client-info.txt"
			}

		docker_app_uninstall() {
			cd /home/docker/shadowsocks-rust/ && docker compose down --rmi all
