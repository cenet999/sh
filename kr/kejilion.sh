...
ldnmp_Proxy() {
	...
	if ! docker exec nginx nginx -s reload; then
		echo -e "${gl_hong}실수:${gl_bai}Nginx 容器重载失败。"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}43.  ${gl_bai}V2Ray-에이전트 설치 스크립트"
	  echo -e "${gl_kjlan}------------------------"
...
echo "역방향 생성 설치 no SSL k fd-nossl |k rp-nossl |k fd xxx.com 127.0.0.1 3000 no_ssl"
