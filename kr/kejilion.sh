...
UserLicenseAgreement() {
	...
	echo "사용자 라이센스 계약: https://github.com/cenet999/sh/blob/main/LICENSE"
	echo -e "----------------------"
...
install_ssltls() {
	  ...
	  if [ ! -f "$file_path" ]; then
			...
			if [[ ($yuming =~ $ipv4_pattern || $yuming =~ $ipv6_pattern) ]]; then
				...
				echo "일반 도메인 이름이 감지되었습니다.$yuming, 자체 서명된 인증서가 생성되었습니다. 브라우저에서 신뢰할 수 있는 인증서가 필요한 경우 DNS 확인에 적용된 도메인 이름 인증서를 수동으로 가져오세요."
			else
...
install_ssltls_text() {
	...
	echo "공개 키: /etc/letsencrypt/live/$cert_name/fullchain.pem"
	echo "개인 키: /etc/letsencrypt/live/$cert_name/privkey.pem"
	echo ""
}
...
ldnmp_Proxy() {
	...
	sed -i "s/# 동적으로 추가/$upstream_servers/g" "/home/web/conf.d/$site_name.conf"
	...
	if ! docker exec nginx nginx -s reload; then
		...
		echo "Docker 애플리케이션을 다시 생성할 때 이 문제가 발생하면 다음을 실행하십시오."
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "고급 게임플레이: https://github.com/cenet999/sh"
  	  echo "------------------------------------------------"
...
v2ray_agent_install() {
	send_stats "V2Ray-Agent 설치 스크립트 설치"
	root_use
	install wget
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}39.  ${gl_bai}사이트 역방향 프록시-IP+포트+SSL 없음"
	echo -e "${gl_huang}------------------------"
	...
	case $sub_choice in
	  ...
	  sed -i '/# SSL 인증서 구성/d' /home/web/conf.d/$yuming.conf
	  sed -i '/# HTTP가 HTTPS/,+3d로 리디렉션됩니다.' /home/web/conf.d/$yuming.conf
	  ...
	  echo -e "일반 도메인 이름의 예:${gl_huang}*.name.com${gl_bai}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}119. ${color119}Nginx 정적 애플리케이션 스테이션"
	  echo -e "${gl_kjlan}-------------------------"
...
EOF
	  ...
	  119|nginx-static|nginx|static-nginx)
			...
			local app_id="119"
			local app_name="Nginx 정적 애플리케이션 스테이션"
			local app_text="정적 페이지, 프런트엔드 패키지 제품 및 단일 페이지 애플리케이션을 호스팅하는 데 적합합니다. 도메인 이름 디렉터리에 따라 독립적으로 배포하면 관리가 더 편리해집니다."
			local app_url="미러 주소: https://hub.docker.com/_/nginx"
...
EOF
			...
			echo "설치 완료"
			check_docker_app_ip
			echo "웹사이트 디렉토리:${app_workdir}/html"
			echo "파일 작성:${app_workdir}/docker-compose.yml"
		}
		...
		docker_app_update() {
			...
			if [ -f "${domain_file}" ]; then
				...
				read -e -p "도메인 이름을 입력하세요:" site_domain
			fi
			if [ -z "${site_domain}" ]; then
				echo "도메인 이름은 비워둘 수 없습니다."
				return 1
			fi
		...
		read -e -p "도메인 이름을 입력하십시오(디렉터리 이름 및 사이트 이름의 경우):" site_domain_input
		read -e -p "기본적으로 사용하려면 외부 액세스 포트를 입력하고 Enter를 누르세요.${docker_port}포트:" app_port
		if [ -z "${site_domain_input}" ]; then
			...
			if ss -tuln | grep -q ":$docker_port "; then
				echo -e "${gl_hong}실수:${gl_bai}포트$docker_port이미 사용 중입니다. 포트를 변경하세요."
				break_end
			else
				setup_docker_dir
				check_disk_space $app_size /home/docker
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}43.  ${gl_bai}V2Ray-Agent 설치 스크립트"
	  echo -e "${gl_kjlan}------------------------"
...
echo -e "${gl_kjlan}블로그:${gl_bai}https://github.com/cenet999/sh         ${gl_kjlan}소프트웨어 센터:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}스크립트 공식 웹사이트:${gl_bai}https://github.com/cenet999/sh            ${gl_kjlan}GitHub 주소:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "변경 로그"
	echo "------------------------"
	echo "모든 로그:${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
...
echo "역방향 생성 설치 SSL 없음 k fd-nossl |k rp-nossl |k fd xxx.com 127.0.0.1 3000 no_ssl"
