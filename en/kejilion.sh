...
UserLicenseAgreement() {
	...
	echo "User License Agreement: https://github.com/cenet999/sh/blob/main/LICENSE"
	echo -e "----------------------"
...
ldnmp_Proxy() {
	...
	if ! docker exec nginx nginx -s reload; then
		...
		echo "If you encounter this problem when regenerating a Docker application, please execute:"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "Advanced gameplay: https://github.com/cenet999/sh"
  	  echo "------------------------------------------------"
...
v2ray_agent_install() {
	send_stats "Install V2Ray-Agent installation script"
	root_use
	install wget
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  sed -i '/# SSL certificate configuration/d' /home/web/conf.d/$yuming.conf
	  sed -i '/# HTTP redirect to HTTPS/,+3d' /home/web/conf.d/$yuming.conf
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}119. ${color119}Nginx static application station"
	  echo -e "${gl_kjlan}-------------------------"
...
EOF
	  ...
	  119|nginx-static|nginx|static-nginx)
			...
			local app_id="119"
			local app_name="Nginx static application station"
			local app_text="It is suitable for hosting static pages, front-end packaged products and single-page applications. Independent deployment according to the domain name directory is more convenient for management."
			local app_url="Mirror address: https://hub.docker.com/_/nginx"
...
EOF
			...
			echo "Installation completed"
			check_docker_app_ip
			echo "Website directory:${app_workdir}/html"
			echo "Compose file:${app_workdir}/docker-compose.yml"
		}
		...
		docker_app_update() {
			...
			if [ -f "${domain_file}" ]; then
				...
				read -e -p "Please enter domain name:" site_domain
			fi
		...
		read -e -p "Please enter domain name (for directory name and site name):" site_domain_input
		read -e -p "Please enter the external access port and press Enter to use it by default.${docker_port}port:" app_port
		if [ -z "${site_domain_input}" ]; then
			...
			if ss -tuln | grep -q ":$docker_port "; then
				echo -e "${gl_hong}mistake:${gl_bai}port$docker_portAlready occupied, please change a port"
				break_end
			else
				setup_docker_dir
				check_disk_space $app_size /home/docker
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}43.  ${gl_bai}V2Ray-Agent installation script"
	  echo -e "${gl_kjlan}------------------------"
...
echo -e "${gl_kjlan}blog:${gl_bai}https://github.com/cenet999/sh         ${gl_kjlan}Software Center:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}Script official website:${gl_bai}https://github.com/cenet999/sh            ${gl_kjlan}GitHub address:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "Change log"
	echo "------------------------"
	echo "All logs:${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
