...
UserLicenseAgreement() {
	...
	echo "Пользовательский лицензионный соглашение: https://github.com/cenet999/sh/blob/main/LICENSE"
	echo -e "----------------------"
...
install_ldnmp_conf() {
  ...
  wget -O /home/web/redis/valkey.conf ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/valkey.conf
...
echo -e "${gl_kjlan}Блог:${gl_bai}https://github.com/cenet999/sh          ${gl_kjlan}Центр программного обеспечения:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "------------------------"
	echo "Все журналы:${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt"
	echo "------------------------"

	curl -s ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/cn/kejilion.sh && chmod +x kejilion.sh
			else
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion.sh && chmod +x kejilion.sh
			fi
			canshu_v6
			CheckFirstRun_true
			yinsiyuanquan2
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/cenet999/sh/main/kejilion.sh && chmod +x kejilion.sh && sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ./kejilion.sh"
			elif [ -n "$ipv6_address" ]; then
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/cenet999/sh/main/kejilion.sh && chmod +x kejilion.sh && sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ./kejilion.sh"
			else
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/cenet999/sh/main/kejilion.sh && chmod +x kejilion.sh"
			fi
			check_crontab_installed
			(crontab -l | grep -v "kejilion.sh") | crontab -
