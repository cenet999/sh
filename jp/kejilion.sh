...
ldnmp_Proxy() {
	...
	sed -i "s/# 動的に追加/$upstream_servers/g" "/home/web/conf.d/$site_name.conf"
...
EOF
			...
			echo "ファイルを作成します:${app_workdir}/docker-compose.yml"
		}
		...
		read -e -p "デフォルトで使用するには、外部アクセス ポートを入力して Enter を押してください。${docker_port}ポート：" app_port
