...
install_ssltls() {
	  ...
	  if [ ! -f "$file_path" ]; then
			...
			if [[ ($yuming =~ $ipv4_pattern || $yuming =~ $ipv6_pattern) ]]; then
				...
				echo "汎用ドメイン名が検出されました$yuming、自己署名証明書が生成されました。ブラウザーの信頼された証明書が必要な場合は、DNS 検証に適用されたドメイン名証明書を手動でインポートしてください。"
			else
...
install_ssltls_text() {
	...
	echo "公開キー: /etc/letsencrypt/live/$cert_name/fullchain.pem"
	echo "秘密鍵: /etc/letsencrypt/live/$cert_name/privkey.pem"
	echo ""
}
...
ldnmp_Proxy() {
	...
	if is_no_ssl_mode "$ssl_mode"; then
		webname="リバース プロキシ - IP + ポート + SSL なし"
	fi
	send_stats "インストール$webname"
	echo "導入を開始する$webname"
	if [ -z "$yuming" ]; then
	...
	if ! docker exec nginx nginx -s reload; then
		...
		echo "Docker アプリケーションを再生成するときにこの問題が発生した場合は、次のコマンドを実行してください。"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "高度なゲームプレイ: https://github.com/cenet999/sh"
  	  echo "------------------------------------------------"
...
v2ray_agent_install() {
	send_stats "V2Ray-Agent インストール スクリプトをインストールする"
	root_use
	install wget
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}39.  ${gl_bai}サイト リバース プロキシ - IP + ポート + SSL なし"
	echo -e "${gl_huang}------------------------"
	...
	case $sub_choice in
	  ...
	  sed -i '/# SSL 証明書の設定/d' /home/web/conf.d/$yuming.conf
	  sed -i '/# HTTPS/,+3d への HTTP リダイレクト' /home/web/conf.d/$yuming.conf
	  ...
	  echo -e "一般的なドメイン名の例:${gl_huang}*.name.com${gl_bai}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}117. ${color117}SQL Server データベース サービス${gl_kjlan}118. ${color118}re:Director重定向服务"
	  echo -e "${gl_kjlan}119. ${color119}Nginx 静的アプリケーション ステーション"
	  echo -e "${gl_kjlan}-------------------------"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  116|shadowsocks-rust|ssserver-rust|shadowsocks)
			...
			local app_text="軽量のプロキシ サービス。モバイル クライアントまたはコンピュータ クライアントへの直接接続に適しています。これは Web ページのパネルではありません。クライアントは、サーバーの IP、ポート、パスワード、および暗号化方式を入力するだけで済みます。"
...
EOF
				...
				echo "暗号化:${ss_method}"
				echo "注: これは Web アプリケーションではなくプロキシ サービスであり、Web ページから元に戻すことはできません。"
			}
...
EOF
	  ...
	  119|nginx-static|nginx|static-nginx)
			...
			local app_id="119"
			local app_name="Nginx 静的アプリケーション ステーション"
			local app_text="静的ページ、フロントエンドのパッケージ製品、シングルページ アプリケーションのホストに適しています。ドメイン名ディレクトリに応じた独立した展開は、管理上より便利です。"
			local app_url="ミラーアドレス: https://hub.docker.com/_/nginx"
...
EOF
			...
			echo "インストール完了"
			check_docker_app_ip
			echo "ウェブサイトのディレクトリ:${app_workdir}/html"
		...
		docker_app_update() {
			...
			if [ -f "${domain_file}" ]; then
				...
				read -e -p "ドメイン名を入力してください:" site_domain
			fi
			if [ -z "${site_domain}" ]; then
				echo "ドメイン名を空にすることはできません"
				return 1
			fi
		...
		read -e -p "ドメイン名を入力してください (ディレクトリ名とサイト名):" site_domain_input
		read -e -p "デフォルトで使用するには、外部アクセス ポートを入力して Enter を押してください。${docker_port}端口: " app_port
		if [ -z "${site_domain_input}" ]; then
			...
			if ss -tuln | grep -q ":$docker_port "; then
				echo -e "${gl_hong}間違い：${gl_bai}ポート$docker_portすでに占有されています。ポートを変更してください"
				break_end
			else
				setup_docker_dir
				check_disk_space $app_size /home/docker
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}43.  ${gl_bai}V2Ray-Agent インストール スクリプト"
	  echo -e "${gl_kjlan}------------------------"
...
while true; do
	  ...
	  echo -e "${gl_kjlan}14. ${gl_bai}ドッカーをインストールする${gl_kjlan}15. ${gl_bai}安装BBR3              ${gl_kjlan}16. ${gl_bai}1Gの仮想メモリを設定する"
...
echo -e "${gl_kjlan}ブログ:${gl_bai}https://github.com/cenet999/sh         ${gl_kjlan}ソフトウェアセンター:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}スクリプト公式サイト：${gl_bai}https://github.com/cenet999/sh            ${gl_kjlan}GitHub アドレス:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "変更ログ"
	echo "------------------------"
	echo "すべてのログ:${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
...
echo "逆生成のインストール no SSL k fd-nossl |k rp-nossl |k fd xxx.com 127.0.0.1 3000 no_ssl"
