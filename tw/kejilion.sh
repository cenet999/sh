...
UserLicenseAgreement() {
	...
	echo "使用者授權協議: https://github.com/cenet999/sh/blob/main/LICENSE"
	echo -e "----------------------"
...
install_ssltls() {
	  ...
	  if [ ! -f "$file_path" ]; then
			...
			if [[ ($yuming =~ $ipv4_pattern || $yuming =~ $ipv6_pattern) ]]; then
				...
				echo "偵測到泛域名$yuming，已生成自簽證書。如需瀏覽器可信任證書，請手動匯入 DNS 驗證申請的泛網域證書。"
			else
...
install_ssltls_text() {
	...
	echo "公鑰: /etc/letsencrypt/live/$cert_name/fullchain.pem"
	echo "私鑰: /etc/letsencrypt/live/$cert_name/privkey.pem"
	echo ""
}
...
ldnmp_Proxy() {
	...
	sed -i "s/# 動態添加/$upstream_servers/g" "/home/web/conf.d/$site_name.conf"
	...
	if ! docker exec nginx nginx -s reload; then
		echo -e "${gl_hong}錯誤:${gl_bai}Nginx 容器重載失敗。"
		echo "如果你是反代 Docker 應用程式時遇到這個問題，請先執行："
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "進階玩法: https://github.com/cenet999/sh"
  	  echo "------------------------------------------------"
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}39.  ${gl_bai}站點反向代理-IP+連接埠+無SSL"
	echo -e "${gl_huang}------------------------"
	...
	case $sub_choice in
	  ...
	  sed -i '/# SSL 憑證設定/d' /home/web/conf.d/$yuming.conf
	  sed -i '/# HTTP 重新導向到 HTTPS/,+3d' /home/web/conf.d/$yuming.conf
	  ...
	  echo -e "泛網域名稱範例:${gl_huang}*.name.com${gl_bai}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}115. ${color115}V2RayA代理程式管理面板${gl_kjlan}116. ${color116}Shadowsocks Rust代理服務端"
	  echo -e "${gl_kjlan}117. ${color117}SQL Server資料庫服務${gl_kjlan}118. ${color118}re:Director重新導向服務"
	  echo -e "${gl_kjlan}119. ${color119}Nginx靜態應用站"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}第三方應用程式列表"
  	  echo -e "${gl_kjlan}想要讓你的應用程式出現在這裡？查看開發者指南:${gl_huang}https://github.com/cenet999/sh/tree/main/apps${gl_bai}"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  115|v2raya|V2RayA|v2rayA)
			...
			local app_id="115"
			local app_name="V2RayA代理程式管理面板"
			local app_text="一個自架代理管理面板，適合用網頁管理 v2ray/xray 節點和系統代理程式。"
			local app_url="項目地址:${gh_https_url}github.com/v2rayA/v2rayA"
	  ...
	  116|shadowsocks-rust|ssserver-rust|shadowsocks)
			...
			local app_id="116"
			local app_name="Shadowsocks Rust代理服務端"
			local app_text="輕量代理服務，適合給手機用戶端或電腦用戶端直連使用。它不是網頁面板，客戶端填伺服器IP、連接埠、密碼、加密方式即可。"
			local app_url="項目地址:${gh_https_url}github.com/shadowsocks/shadowsocks-rust"
			...
			docker_app_install() {
				...
				read -e -p "設定 Shadowsocks 密碼，回車自動產生隨機密碼:" ss_password
				local ss_password=${ss_password:-$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 16)}

				read -e -p "設定加密方式，回車預設 chacha20-ietf-poly1305:" ss_method
...
EOF
				...
				echo "客戶端這樣填："
				echo "地址：你的服务器IP或域名"
				echo "連接埠：${docker_port}"
				echo "密碼：${ss_password}"
				echo "加密：${ss_method}"
				echo "說明：這是代理服務，不是網頁應用，不能走網頁反代。"
			}
			...
			docker_app_update() {
				...
				clear
				echo "更新完成"
				echo "當前客戶端參數保存在:${app_workdir}/client-info.txt"
			}
	  ...
	  117|sqlserver|mssql|SQLServer|SQLSERVER)
			...
			local app_id="117"
			local app_name="SQL Server資料庫服務"
			local app_text="微軟 SQL Server 資料庫。它不是網頁面板，適合給程式、Navicat、DBeaver 這類客戶端連線使用。"
			local app_url="項目地址:${gh_https_url}github.com/microsoft/mssql-docker"
			...
			docker_app_install() {
				...
				read -e -p "設定 SA 密碼，回車預設使用 YourStrong!Passw0rd:" sa_password
...
EOF
			...
			docker_app_update() {
				...
				clear
				echo "更新完成"
				echo "連線資訊保存在:${app_workdir}/client-info.txt"
			}
	  ...
	  118|re-director|redirector|redirect)
			...
			local app_id="118"
			local app_name="re:Director重新導向服務"
			local app_text="一個自架的重定向管理服務，適合統一管理短連結、域名跳轉和 301/302/307/308 重定向。"
			local app_url="項目地址:${gh_https_url}github.com/re-Director/re-director"
			...
			docker_app_install() {
				...
				echo "安裝完成"
				check_docker_app_ip
				echo "提示：把需要跳轉的網域解析到本機，再到面板建立對應的重定向規則即可。"
			}
	  ...
	  119|nginx-static|nginx|static-nginx)
			...
			local app_id="119"
			local app_name="Nginx靜態應用站"
			local app_text="適合託管靜態頁面、前端打包產物和單頁應用，依網域目錄獨立部署更方便管理。"
			local app_url="鏡像位址: https://hub.docker.com/_/nginx"
...
EOF
			...
			echo "安裝完成"
			check_docker_app_ip
			echo "網站目錄:${app_workdir}/html"
			echo "Compose檔:${app_workdir}/docker-compose.yml"
		}
		...
		read -e -p "請輸入網域名稱（用於目錄名稱和網站名稱）:" site_domain_input
		read -e -p "請輸入對外訪問端口，回車預設使用${docker_port}連接埠:" app_port
		if [ -z "${site_domain_input}" ]; then
			...
			if ss -tuln | grep -q ":$docker_port "; then
				echo -e "${gl_hong}錯誤:${gl_bai}連接埠$docker_port已被佔用，請更換一個端口"
				break_end
			else
				setup_docker_dir
				check_disk_space $app_size /home/docker
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}43.  ${gl_bai}V2Ray-Agent安裝腳本"
	  echo -e "${gl_kjlan}------------------------"
...
echo -e "${gl_kjlan}部落格:${gl_bai}https://github.com/cenet999/sh         ${gl_kjlan}軟體中心:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}腳本官網:${gl_bai}https://github.com/cenet999/sh            ${gl_kjlan}GitHub地址:${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日誌"
	echo "------------------------"
	echo "全部日誌:${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt"
	echo "------------------------"

	curl -s --max-time 15 ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion_sh_log.txt | tail -n 30
	# 只下載前5行取得版本號，避免下載整個腳本
	local sh_v_new=$(curl -s --max-time 15 -r 0-200 ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion.sh | grep -o 'sh_v="[0-9.]*"' | head -1 | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local download_url
			if [ "$country" = "CN" ]; then
				download_url="${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/cn/kejilion.sh"
			else
				download_url="${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion.sh"
			fi
		...
		2)
			...
			SH_Update_task="cd ~ && tmp=\$(mktemp ~/kejilion_tmp.XXXXXX) && curl -sS --max-time 60 --fail -o \"\$tmp\" ${cron_proxy}raw.githubusercontent.com/cenet999/sh/main/kejilion.sh && [ -s \"\$tmp\" ] && head -1 \"\$tmp\" | grep -q '^#!/bin/bash' && cp -f ~/kejilion.sh ~/kejilion.sh.bak 2>/dev/null && chmod +x \"\$tmp\" && mv -f \"\$tmp\" ~/kejilion.sh"
			# 追加設定恢復
...
echo "安裝反代無SSL k fd-nossl |k rp-nossl |k fd xxx.com 127.0.0.1 3000 no_ssl"
